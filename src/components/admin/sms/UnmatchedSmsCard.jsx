        ...
        {resolutionState === 'matched' ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
              <Button onClick={() => matchMutation.mutate()} disabled={!selectedLeadId || matchMutation.isPending || noMatchMutation.isPending} className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white disabled:opacity-50">
                {matchMutation.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
                Attach to selected lead
              </Button>
              <Button variant="outline" onClick={() => noMatchMutation.mutate()} disabled={matchMutation.isPending || noMatchMutation.isPending} className="border-white/10 text-white hover:bg-white/5">
