    ...
    <Card className="bg-[#12121a] border-white/5">
      <CardContent className="p-6 space-y-4">
        ...
        {orderedLogs.length === 0 ? (
          ...
          const highIntentTags = getHighIntentTags(log);
          const isHighIntent = direction === 'inbound' && highIntentTags.length > 0;
          ...
          return (
            <div key={log.id} className={isHighIntent ? 'rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] px-4 py-4 space-y-3' : 'rounded-2xl border border-white/5 bg-white/[0.03] px-4 py-4 space-y-3'}>
              <div className="flex flex-wrap items-center gap-2">
                ...
                {isHighIntent && <Badge className="bg-amber-500/10 text-amber-300 border-amber-500/20">high intent</Badge>}
                {wasManuallyMatched && <Badge className="bg-cyan-500/10 text-cyan-300 border-cyan-500/20">manually matched</Badge>}
                {tags.map((tag) => <Badge key={tag} className={HIGH_INTENT_TAGS.has(tag) ? 'bg-amber-500/10 text-amber-300 border-amber-500/20' : 'bg-fuchsia-500/10 text-fuchsia-300 border-fuchsia-500/20'}>{tag}</Badge>)}
              </div>
