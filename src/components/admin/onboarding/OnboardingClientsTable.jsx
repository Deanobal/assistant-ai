      ...
      <CardContent className="p-0 overflow-x-auto">
        <table className="w-full min-w-[980px] text-sm">
          <thead className="bg-white/[0.03] text-gray-400">
            <tr>
              {['Business Name', 'Contact Name', 'Industry', 'Plan', 'Status', 'Progress %', 'Assigned Owner', 'Last Activity', 'View Client'].map((label) => (
              ...
              <tr key={client.id} className="border-t border-white/5">
                ...
                <td className="px-5 py-4">
                  <Link to={`/ClientWorkspace?id=${client.id}`}>
                    <Button size="sm" className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white">View Client</Button>
                  </Link>
                </td>
              </tr>
            ))}
