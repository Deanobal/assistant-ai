  ...
  useEffect(() => {
    ...
    const loadStatus = async () => {
      try {
        ...
        if (!response.ok) throw new Error(data?.error || data?.details || 'Unable to confirm your Stripe payment yet.');
        setState({ loading: false, error: '', data });
      } catch (error) {
        setState({ loading: false, error: error.message || 'Unable to confirm your Stripe payment yet.', data: null });
      }
    };
  ...
  return (
    <Card className="bg-[#12121a] border-white/5">
      <CardContent className="p-8 text-center space-y-4">
        {state.loading ? (
        ...
        ) : state.data?.payment_status === 'paid' || state.data?.status === 'complete' || state.data?.billing_status === 'active' ? (
          <>
            ...
            <p className="text-gray-400 max-w-2xl mx-auto">Your {state.data?.selected_plan || state.data?.plan_name || planName} payment has been confirmed by Stripe, and onboarding is being prepared.</p>
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 text-sm text-gray-300 text-left max-w-2xl mx-auto">
              ...
              <ul className="space-y-1">
                ...
                <li>• Your client, billing, and onboarding records are checked through the native AssistantAI API.</li>
          ...
          <>
            ...
            {state.data?.payment_status && state.data?.payment_status !== 'paid' && (
              <p className="text-sm text-amber-200">Current payment status: {state.data.payment_status}</p>
            )}
          </>
        )}
      </CardContent>
