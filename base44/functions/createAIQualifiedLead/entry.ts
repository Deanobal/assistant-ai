...
function jsonToolResponse(body, status = 200) {
  console.log('Final response:', JSON.stringify({ success: body?.success, error: body?.error, lead_id: body?.lead_id }));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      return jsonToolResponse({ success: true, message: 'createAIQualifiedLead reachable. Use POST to create leads.' });
    }
