...
function buildEventPayload(entityName, eventType, data, oldData) {
  if (!isLeadEntity(entityName) || !data?.id) return [];
...
async function invokeAdminAlert(base44, reqUrl, payload) {
  let lastError = null;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      return await runAdminAlert(base44, reqUrl, payload);
    } catch (error) {
      lastError = error;
    }
  }
...
Deno.serve(async (req) => {
  try {
    ...
    for (const def of eventDefs) {
      ...
      const adminResponse = await invokeAdminAlert(base44, req.url, {
