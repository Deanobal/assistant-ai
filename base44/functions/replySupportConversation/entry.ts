  ...
  try {
    ...
    if (aiResult && ['human_required', 'escalated'].includes(aiResult.ai_mode)) {
      await base44.asServiceRole.functions.invoke('sendAdminAlert', {
        ...
        priority: aiResult.urgency_level === 'urgent' ? 'high' : 'normal',
        smsMessage: `${aiResult.ai_mode === 'escalated' ? 'Urgent chat:' : 'Chat escalation:'} ${name || conversation.visitor_name || 'Visitor'} - ${conversation.visitor_phone || 'No phone'} - ${aiResult.enquiry_category}`.slice(0, 160),
        metadata: {
          conversation_id: conversation.id,
          admin_link: `/SupportInbox`,
