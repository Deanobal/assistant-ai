...
Deno.serve(async (req) => {
  try {
    ...
    const conversation = await base44.asServiceRole.entities.SupportConversation.create({
      ...
      visitor_name: name,
      visitor_email: email,
      visitor_phone: mobile || aiResult.captured_phone || '',
      subject,
      ...
      detected_intents: aiResult.detected_intents || [],
      pricing_shown: aiResult.pricing_shown || false,
    ...
    if (['human_required', 'escalated'].includes(aiResult.ai_mode)) {
      alertResults = await createPublicAlert(base44, {
        ...
        title: aiResult.closing_mode ? 'Closing lead needs immediate call' : aiResult.high_value_lead ? 'High-value lead needs reply' : (aiResult.ai_mode === 'escalated' || aiResult.enquiry_category === 'sales' ? 'High-intent chat needs reply' : 'Chat needs human reply'),
        message: aiResult.ai_summary || preview,
        actorEmail: email,
        uniqueKey: `chat_handover_create:${conversation.id}:${aiResult.ai_mode}:${now}`,
        priority: aiResult.closing_mode || aiResult.urgency_level === 'urgent' ? 'urgent' : (aiResult.high_value_lead || aiResult.urgency_level === 'high' ? 'high' : 'normal'),
        ...
        metadata: {
          ...
          email,
          mobile_number: mobile || aiResult.captured_phone || '',
          enquiry_category: aiResult.enquiry_category,
          urgency_level: aiResult.urgency_level,
          issue_category: aiResult.issue_category || null,
          confidence_level: aiResult.confidence_level || null,
